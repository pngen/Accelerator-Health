...
int main(){
  auto c=clk(); HealthStore s(c,{},1); auto id=s.registerDevice(DeviceUuid{7},NodeId{1}); setup(s,id);
  s.beginQuarantine(id,QuarantineAuthority::FAULT,"fatal","DEVICE_LOST");
  s.clearQuarantine(id); s.beginRecovery(id);
  std::printf("recovering: state=%s\n",to_string(s.snapshot(id).state));
  ValidationRecord v; v.id=ValidationRunId{9};v.accelerator=id;v.timestamp=Timestamp{50};v.profile=ValidationProfile::FULL;v.depth=ValidationDepth::FULL;v.passed=true;v.validationGeneration=ValidationGeneration{2};
  auto sn=s.revalidate(id,v);
  std::printf("revalidated: state=%s readiness=%s\n",to_string(sn.state),to_string(sn.readiness));
  return 0;
}