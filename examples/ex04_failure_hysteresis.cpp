...
int main(){
  auto c=clk(); HealthStore s(c,{},1); auto id=s.registerDevice(DeviceUuid{4},NodeId{1}); setup(s,id);
  for(int n=1;n<=3;++n){ Fault f; f.id=FaultId{static_cast<std::uint64_t>(n)};f.accelerator=id;f.type=FaultType::TRANSFER_FAILURE;f.severity=Severity::DEGRADED;f.faultClass=FaultClass::TRANSIENT;f.transient=true;f.recoverable=true;f.timestamp=Timestamp{30+n}; s.recordFault(f); std::printf("after %d failures: state=%s\n",n,to_string(s.snapshot(id).state)); }
  return 0;
}