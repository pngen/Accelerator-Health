...
int main(){
  auto c=clk(); HealthStore s(c,{},1); auto id=s.registerDevice(DeviceUuid{5},NodeId{1}); setup(s,id);
  Fault f; f.id=FaultId{1};f.accelerator=id;f.type=FaultType::DEVICE_LOST;f.severity=Severity::FATAL;f.faultClass=FaultClass::FATAL;f.fatal=true;f.timestamp=Timestamp{30};
  s.recordFault(f);
  std::printf("after DEVICE_LOST: state=%s readiness=%s\n",to_string(s.snapshot(id).state),to_string(s.snapshot(id).readiness));
  return 0;
}