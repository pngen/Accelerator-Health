...
int main(){
  auto c=clk(); HealthStore s(c,{},1); auto id=s.registerDevice(DeviceUuid{13},NodeId{1}); setup(s,id);
  auto bytes=s.encodeState();
  HealthStore s2(clk(),{},5); std::string err;
  if(!HealthStore::decode(bytes,s2,&err)){ std::printf("decode failed: %s\n",err.c_str()); return 1; }
  auto sn=s2.snapshot(id);
  std::printf("after recovery: state=%s readiness=%s\n",to_string(sn.state),to_string(sn.readiness));
  return 0;
}