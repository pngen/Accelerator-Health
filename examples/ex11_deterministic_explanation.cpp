...
int main(){
  auto c=clk(); HealthStore s(c,{},1); auto id=s.registerDevice(DeviceUuid{11},NodeId{1}); setup(s,id);
  auto a=s.assessmentOf(id);
  auto d=AssessmentEngine{}.explain(a);
  std::printf("%s",d.json.c_str());
  std::printf("\ndigest=%s\n",d.digest.c_str());
  return 0;
}