...
int main(){
  auto c=clk(); HealthStore s(c,{},1); auto id=s.registerDevice(DeviceUuid{6},NodeId{1}); setup(s,id);
  s.beginQuarantine(id,QuarantineAuthority::FAULT,"fatal fault","DEVICE_LOST");
  auto sn=s.snapshot(id);
  std::printf("quarantined=%d state=%s readiness=%s\n",sn.quarantine&&sn.quarantine->active?1:0,to_string(sn.state),to_string(sn.readiness));
  return 0;
}